...
function start() {
  ...
  --http \
  --http.api "eth,net,web3,admin,debug,trace,txpool" \
  --ws.addr 0.0.0.0 --ws.port $(( 8546 + $index )) --ws.api "eth,net,web3,admin,debug,trace,txpool" \
...
clean)
    ...
    ;;
*)
    echo "Usage: ctc_archive_node.sh start|stop|restart|clean nodeIndex syncmode"
    echo "like: ctc_archive_node.sh start 1 snap, it will startup a snapsync node1"
    ;;
esac
